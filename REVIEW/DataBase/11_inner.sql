-- 0. JOIN이란?
-- 	두개 이상의 테이블을 묶어서 하나의 결과 집합으로 출력하는 것
-- 
-- 1. INNER JOIN의 구조
-- 	SELECT 컬럼1, 컬럼2
...
-- 		ON 조인 조건
-- 	[WHERE 검색조건]
--		[ where /group by / order by]순으로 작성해야 함. 
-- 	from 테이블과 inner join 테이블을 먼저 작성하고 select를 작성하는 것이 편함. 
...
INNER JOIN dept_emp demp
		...
		ORDER BY emp.emp_no DESC LIMIT 10;
...
-- 	- 실무에서 사용되는 경우는 거의 없고, 가끔 테스트용 테이터를 생성하기 위해 사용 될 수도 있음.
-- SELECT 컬럼1, 컬럼2 ...
-- FROM 테이블1
-- 	CROSS JOIN 테이블2
-- WHERE 검색조건;
