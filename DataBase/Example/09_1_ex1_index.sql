...
-- 	- 전반적인 시스템의 부하 감소
--  - 인덱스된 컬럼으로 정렬 명령을 내리면 정렬비용 감소

-- 3. INDEX의 단점
-- 	- 인덱스를 관리하기 위해 DB의 약 10%에 해당하는 추가 저장공간이 필요
-- 	- 인덱스를 관리하기 위해 추가 작업이 필요 (추가, 삭제, 유지....)
...
-- 	- 인덱스를 추가 했다면 대량의 데이터로 해당 테이블의 CRUD를 테스트
                            10만~30만                crud : create (생성), read (읽기), update(갱신), delete(삭제)
-- 	- 사용하지 않는 인덱스는 제거
-- 	- FK를 지정한 열은 자동으로 FK 인덱스가 생성 (*DBMS에 따라 상이하므로 주의)
--      mariadb는 생성해줌, oracle은 생성x

-- 5. 인덱스를 사용하기 좋은 상황(회원정보 테이블에 가장 사용하기 좋음. )
-- 	- 규모가 작지 않은 테이블
-- 	- INSERT, UPDATE, DELETE가 자주 발생하지 않는 칼럼
-- 	- JOIN(속도저하의 주범)이나 WHERE 또는 ORDER BY에 자주 사용되는 칼럼
-- 	- 데이터의 중복도가 낮은 칼럼
